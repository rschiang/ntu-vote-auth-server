...
def to_student_id(internal_id):
    ...
    status_ok = resp_entity.find('WEBOK')
    error = resp_entity.find('ERROR')
    student_id = resp_entity.find('STUID').text
    student_type = resp_entity.find('STUTYPE')
    in_campus = resp_entity.find('INCAMPUS')
    college = resp_entity.find('COLLEGE')
